    ...
    <Section id="education" title="Education">
      <div className="space-y-6">
        {EDUCATION.map((item) => (
          ...
          >
            <h3 className="text-xl font-bold text-text-primary dark:text-dark-text-primary transition-colors duration-300 group-hover:text-accent">{item.degree}</h3>
            <p className="text-accent mt-1 font-semibold">{item.institution}</p>
            <p className="text-text-secondary dark:text-dark-text-secondary mt-2">{item.details}</p>
          </div>
        ))}
      </div>
    </Section>
