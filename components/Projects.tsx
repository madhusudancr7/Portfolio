      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
        {PROJECTS.length > 0 ? PROJECTS.map((project, index) => (
          <div 
            ...
            className="bg-card-bg dark:bg-dark-card-bg p-5 md:p-8 rounded-xl border border-border-color dark:border-dark-border-color flex flex-col justify-between group hover:border-accent transition-all duration-300 backdrop-blur-sm hover:shadow-xl hover:shadow-accent/30 hover:-translate-y-2 hover:scale-[1.02] opacity-0 animate-fade-in-up"
            style={{ animationDelay: `${0.1 * index}s` }}
          >
            <div>
              <div className="flex justify-between items-start mb-3 md:mb-4">
                <h3 className="text-xl md:text-2xl font-bold text-text-primary dark:text-dark-text-primary group-hover:text-accent transition-colors duration-300">{project.title}</h3>
                <div className="flex space-x-3 md:space-x-4 flex-shrink-0 ml-4">
                  {project.links.github && (
                      <a href={project.links.github} data-interactive="true" target="_blank" rel="noopener noreferrer" className="text-text-secondary dark:text-dark-text-secondary hover:text-accent transition-colors"><GitHubIcon className="w-5 h-5 md:w-6 md:h-6" /></a>
                  )}
                  {project.links.live && (
                      <a href={project.links.live} data-interactive="true" target="_blank" rel="noopener noreferrer" className="text-text-secondary dark:text-dark-text-secondary hover:text-accent transition-colors"><ExternalLinkIcon className="w-5 h-5 md:w-6 md:h-6" /></a>
                  )}
                </div>
              </div>
              <p className="text-text-secondary dark:text-dark-text-secondary mb-4 md:mb-6 line-clamp-3 md:line-clamp-2 text-base md:text-lg">{project.description}</p>
            </div>
